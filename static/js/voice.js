document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isMobile) {
        ...
        // For mobile devices, add a touch event to ensure audio context is initialized
        document.addEventListener('touchstart', function() {
            if (speechSynthesis && !speechSynthesis.speaking) {
                // Create and immediately cancel a silent utterance to initialize audio context
    ...
    function speakText(text, lang) {
        ...
        if (lang === 'hi-IN') {
            ...
            speechUtterance.rate = isMobile ? 0.9 : (lang === 'hi-IN' ? 0.92 : 0.95);
            ...
            speechUtterance.onerror = function(e) {
                ...
                // Attempt recovery based on specific error types
                if (e.error === 'interrupted' || e.error === 'canceled') {
                    console.log('Speech was interrupted, attempting recovery...');
                    setTimeout(() => {
                        // Try restarting speech with a slight delay
    ...
    function speakInChunks(text, lang) {
        ...
        // Split into smaller chunks - mobile devices need shorter chunks
        const maxChunkSize = isMobile ? 50 : (lang === 'hi-IN' ? 100 : 200);
        ...
        function speakNextChunk() {
            if (chunkIndex < chunks.length) {
                ...
                // Adjust parameters - slower rate for better reliability
                utterance.rate = isMobile ? 0.9 : (lang === 'hi-IN' ? 0.92 : 0.95);
                ...
                utterance.onend = function() {
                    chunkIndex++;
                    if (chunkIndex < chunks.length) {
                        // Longer pause between chunks on mobile
                        setTimeout(speakNextChunk, isMobile ? 500 : 300);
                    } else {
                        // All done
                        outputDiv.classList.remove('speaking');
                        speakBtn.classList.remove('active');
                ...
                utterance.onerror = function(e) {
                    ...
                    // On error, try to recover by continuing
                    chunkIndex++;
                    if (chunkIndex < chunks.length) {
                        setTimeout(speakNextChunk, 500); // Longer delay after error
                    } else {
                        outputDiv.classList.remove('speaking');
                        speakBtn.classList.remove('active');
                    }
        ...
        // Monitor and restart if needed with more frequent checks on mobile
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.assistant-message') && !e.target.closest('.thinking')) {
            ...
            // Adjust parameters
            utterance.rate = isMobile ? 0.9 : (langCode === 'hi-IN' ? 0.92 : 0.95);
            utterance.pitch = 0.9;
            utterance.volume = 1.0;
            ...
            utterance.onerror = function(e) {
                console.error('Message speech error:', e);
                msgElement.classList.remove('speaking');
                
                // Try to recover from interrupted speech
                if (e.error === 'interrupted' || e.error === 'canceled') {
                    setTimeout(() => {
                        if (speechSynthesis.pending) speechSynthesis.cancel();
                        speechSynthesis.speak(utterance);
            ...
            // Speak with slight delay for better browser compatibility
            setTimeout(() => {
                speechSynthesis.speak(utterance);
            }, 100);
        }
    ...
    // More aggressive fix, especially for mobile
    const resumeInterval = setInterval(function() {
        if (speechSynthesis.speaking && !document.hidden) {
            // On mobile, need to be more aggressive with restart
            if (isMobile) {
                speechSynthesis.cancel();
                setTimeout(() => {
    ...
    }, isMobile ? 3000 : 5000); // More frequent for mobile
